...
get_customer_segments <- function(k = 4, seed = 123) {
    ...
    customer_trends_tbl <- bike_orderlines_tbl %>%
        ...
        group_by(bikeshop_name, price, model, category_1, 
                 category_2, frame_material) %>%
        summarise(total_quantity = sum(quantity)) %>%
        ungroup() %>%
        
        group_by(bikeshop_name) %>%
        mutate(percent = total_quantity / sum(total_quantity)) %>%
        ungroup()
    
    customer_product_tbl <- customer_trends_tbl %>%
        select(bikeshop_name, model, percent) %>%
        spread(key = model, value = percent, fill = 0)
...
plot_customer_segments <- function(k = 4, seed = 123, interactive = TRUE) {
    ...
    g <- combined_tbl %>%
        ...
        theme_tq() +
        scale_color_tq() +
        labs(title = "Customer Segmentation 2D",
             subtitle = "UMAP 2D with K-Means Cluster") +
...
plot_customer_behavior_by_cluster <- function(top_n_products = 10, 
    ...
    top_n_tbl <- bike_orderlines_tbl %>%
        ...
        group_by(bikeshop_name, model, category_1,
                 category_2, price) %>%
        summarise(total_quantity = sum(quantity)) %>%
        ungroup() %>%
